$(document).ready(function() {
    ...
    $('.tituloSeccionPantalla').text('Informes Diarios Contables');
...
function generarFilaTablaInicial(data){
  ...
  if(data.validado==2){
    fila.find('.diario_modificado').css('text-align','center').append($('<i>').addClass('fa fa-fw fa-check').css('color','#4CAF50'));
  }
  else{
    fila.find('.diario_modificado').css('text-align','center').append($('<i>').addClass('fas fa-fw fa-times').css('color','#D32F2F'));
  }

  fila.find('.imprimirInfoDiario').val(data.id_importacion_diaria_mesas).css('text-align','center');
  fila.find('.modificar').val(data.id_importacion_diaria_mesas).css('text-align','center').attr('data-fecha',data.fecha);
