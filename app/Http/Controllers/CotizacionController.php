...
class CotizacionController extends Controller
{
    ...
    public function _cotizacionesBNA(string $año_mes){
      ...
      //$CC->invalidar($CACHE_CODIGO,$CACHE_SUBCODIGO);
      ...
      $responses = [];//Originalmente se hacia en varios requests... devuelvo un array de length 1
      ...
      foreach($monedas as $nombre => $id){
        $url.='&idMonedaDescarga='.$id;//Esto no es estandar... deberia ser idMonedaDescarga[]= , pero bueno
      }
      ...
      do {
        if(curl_errno($ch)){
          $responses[0] = [500,['CURL_ERROR' => [curl_error($ch)]]];
          curl_close($ch);
          break;
        }
        else if($httpCode != 200){
          $responses[0] = [$httpCode,['RESPONSE_ERROR' => [$httpCode,$response]]];
          curl_close($ch);
          break;
        }
        curl_close($ch);
        ...
        if($header != ['Moneda','Fecha cotizacion','Compra','Venta']){
          $responses[0] = [500,['CODE_ERROR' => ['Header inesperado',implode(';',$header)]]];
          break;
        }
        
        $header_idx = array_flip($header);
        foreach($monedas_front as $mcsv => $mfront){
          foreach($body as $row){
            $mrow = $row[$header_idx['Moneda']];
            if($mrow != $mcsv) continue;
            $responses[$mfront] = $responses[$mfront] ?? [200,['header' => $header,'body' => []]];
            $responses[$mfront][1]['body'][] = $row;
          }
        }
      } while(false);
      ...
      foreach($responses as $mfront => $code_headerbody){
        ...
        foreach($headerbody['body'] as $row){
          ...
          $compra = trim(str_replace('.',',',$row[$header_idx['Compra']]));
          $venta = trim(str_replace('.',',',$row[$header_idx['Venta']]));
          $merged[$fecha_cotizacion][$mfront] = compact('compra','venta');          
        }
      }
