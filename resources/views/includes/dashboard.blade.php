 ...
 $filtrar_permisos = function ($k, &$opciones) use (&$filtrar_permisos, $ac, $id_usuario) {
     ...
     if (!is_null($hijos)) {
         foreach ($hijos as $op => $datos) {
             ...
             if (count($permisos) != 0 && !$ac->usuarioTieneAlgunPermiso($id_usuario, $permisos)) {
                 continue;
             }
 ...
 $simplificar_submenues = function (&$k, &$opciones, $nivel) use (&$simplificar_submenues) {
     ...
     if ($nivel > 1) {
         if (count($nuevas_opciones['hijos']) == 0 && $opciones['link'] == '') {
             //Si no tiene hijos ni tampoco un link, lo saco
             return null;
         }
         if (count($nuevas_opciones['hijos']) == 1) {
             //Le dejo el mismo icono
 ...
 $fondo = 'rgb(38, 50, 56)';
 //Fisico
 $casinos_ids = $usuario['usuario']->casinos
     ->map(function ($c) {
         return $c->id_casino;
     })
     ->toArray();
